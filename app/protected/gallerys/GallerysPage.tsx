...
import PostForm from "./GallerysForm";
...
const GallerysPage = () => {
  ...
  const fetchPosts = async () => {
    ...
    if (error) {
      console.error("Error fetching gallerys:", error.message);
    } else {
      setPosts(data || []);
    }
  };
  ...
  const handleSubmit = async (
    ...
    if (error) {
      console.error("Error creating gallerys:", error.message);
    } else {
      fetchPosts(); // Refresh posts

      setTimeout(() => {
        router.push("/protected/gallerys"); // Redirect to /posts after 2 seconds
      }, 2000);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">เพิ่มภาพกิจกรรม</h1>
