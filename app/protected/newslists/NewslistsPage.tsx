...
const NewslistsPage = () => {
  ...
  const fetchPosts = async () => {
    ...
    if (error) {
      console.error("Error fetching newslists:", error.message);
    } else {
      setPosts(data || []);
    }
  };
  ...
  const handleSubmit = async (
    ...
    if (error) {
      console.error("Error creating newslists:", error.message);
    } else {
      fetchPosts(); // Refresh posts

      setTimeout(() => {
        router.push("/protected/newslists"); // Redirect to /posts after 2 seconds
      }, 2000);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Add Newslists</h1>
      <NewslistsForm onSubmit={handleSubmit} />
